...
class GitHubMiner:
    ...
    def fetch_developers_by_location(self, location: str, max_pages: int) -> tuple[list[dict],list[GitHubUser]]:
        ...
        for page in range(1, max_pages +1):
            ...
            for item in items:
                ...
                item = self.enrich_with_profile(item)
                # enriching with language data
                time.sleep(0.3)
                lang_data = self.fetch_primary_language(item["login"])
                item["primary_language"] = lang_data["primary_language"]
                item["top_languages"] = lang_data["top_languages"]
                # saving raw jsons
    ...
    def enrich_with_profile(self, item: dict) -> dict:
        ...
        resp = requests.get(
            f"https://api.github.com/users/{item['login']}",
            headers=self.headers
        )

        if resp.status_code != 200:
            print(f"Could not fetch profile for {item['login']}: {resp.status_code}")
            return item

        profile = resp.json()
        item["company"] = profile.get("company")
        item["location"] = profile.get("location")
        item["followers"] = profile.get("followers")
        item["public_repos"] = profile.get("public_repos")
        item["bio"] = profile.get("bio")

        time.sleep(0.5)
        return item
    
    def fetch_primary_language(self, login: str) -> dict:
        """fetches public repos and determines the most used language"""
        resp = requests.get(
        ...
        if resp.status_code != 200:
            print(f"Could not fetch repositories for {login}: {resp.status_code}")
            return{"primary_language": None, "top_languages": None}
        
        repos = resp.json()

        # count the languages in the repo 
        language_counts = {}
        for repo in repos:
        ...
        top_3 = ",".join([lang for lang, count in sorted_languages[:3]])

        return {"primary_language": primary, "top_languages": top_3}
...
if __name__ == "__main__":
    miner = GitHubMiner()
    for city in ["Karlsruhe", "Munich", "Heidelberg", "Berlin"]:
